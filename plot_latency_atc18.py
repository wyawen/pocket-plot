...
puts = (put_s3, put_redis, put_pocket_flash)
gets = (get_s3, get_redis, get_pocket_flash)
...
#rects3 =  ax2.bar(ind[2:]+width, [get_pocket_dram-get_pocket_metadata, get_pocket_flash-get_pocket_metadata], width, color = get_c, hatch='.')
rects3 =  ax2.bar(ind[2:], [put_pocket_flash-put_pocket_metadata], width, color = put_c, hatch='/')
rects4 =  ax2.bar(ind[2:]+width, [get_pocket_flash-get_pocket_metadata], width, color = get_c, hatch='.')
'''
...
plt.tight_layout()
plt.savefig("plots/unloaded-latency-atc18.pdf")
